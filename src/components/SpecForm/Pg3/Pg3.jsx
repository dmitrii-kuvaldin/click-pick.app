...
export default function Pg3() {
  ...
  const [skill, setSkill] = useState(['figma', "template"]);
  ...
  function saveComapny() {
    const imgarr = []
    for (let i = 0; i < newimg3.length; i++) {
      imgarr.push(newimg3[i].db)
    }
    if ((imgarr.length > 0) || (compm[0]?.portfolio.length > 0)) {
      ...
      const task = {
        cost: value, price_policy: policy, checkboxes, work_type_id: 9,
      }
  ...
  useEffect(() => {
    localStorage.setItem('spec', JSON.stringify('pg3'))
  }, [])
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      setDone(true)
      setValue(result.data.filter(el => el.work_type_id === 9).slice(-1)[0].cost)
      setPolicy(result.data.filter(el => el.work_type_id === 9).slice(-1)[0].price_policy)
      setTerms(getObjectKeys(result.data.filter(el => el.work_type_id === 9).slice(-1)[0].checkboxes).filter(el => el === '1-3' || el === '3-6' || el === '6-12' || el === 'more')[0])
      setProg(getObjectKeys(result.data.filter(el => el.work_type_id === 9).slice(-1)[0].checkboxes))
      setSkill(getObjectKeys(result.data.filter(el => el.work_type_id === 9).slice(-1)[0].checkboxes))
      setPics(result.data.filter(el => el.work_type_id === 9).slice(-1)[0]?.portfolio)
      setPicload(false)
      setNewimg3([])
      return result
    }
        ...
        <>
          ...
          >
            <div ref={myRef} className={classes.optHeading}>
              ...
              <h3>Портфолио в UI/UX</h3>
            </div>

            {(newimg3.length === 0 && !compm[0]) || (newimg3.length === 0 && pics?.length === 0) ? (
              <label
            ...
            {/* {(!newimg3 && compm[0]?.portfolio.length > 0) || compm[0]?.portfolio.length > 0 ? (
          <label
            className={classes.uploader}
            htmlFor="formFileMultipleNew"
          >
            <img className={classes.iconUploader} src="/images/icons_svg/uploader.svg" alt="Upload" />
            <p>Подгрузить портфолио к старому
              <br /> в UI/UX дизайне
            </p>
            ...
            {newimg3.length > 0 && <SwiperLocal img={newimg3} newimg={newimg3} setNewimg={setNewimg3} />}
            ...
            {(!newimg3 && pics?.length > 0) || pics?.length > 0 ? (
              <div className={classes.btnWrapper}>
          ...
          >
            ...
            <div className={classes.sliderDiv}>
              ...
              <input className={`${classes.inputSlider} ${classes.slider_progress}`} onChange={(e) => setValue(e.target.valueAsNumber)} value={value} type="range" max='10000' min='300' step='100' />
            </div>
          ...
          >
            ...
            <div className={classes.checkboxWrapper}>
              ...
              >{t('formSpec.pg3.progs.figma')}
              </button>
              <button
                name="sketch"
                onClick={skillHandler}
                className={`${skill.includes('sketch') ? classes.checkboxStep : classes.checkboxStepFade}`}
              >{t('formSpec.pg3.progs.sketch')}
              </button>
              <button
                name="balsamiq"
                onClick={skillHandler}
                className={`${skill.includes('balsamiq') ? classes.checkboxStep : classes.checkboxStepFade}`}
              >{t('formSpec.pg3.progs.balsamiq')}
              </button>
              <button
                name="invision"
                onClick={skillHandler}
                className={`${skill.includes('invision') ? classes.checkboxStep : classes.checkboxStepFade}`}
              >{t('formSpec.pg3.progs.invision')}
              </button>
              <button
                name="marvel"
                onClick={skillHandler}
                className={`${skill.includes('marvel') ? classes.checkboxStep : classes.checkboxStepFade}`}
              >{t('formSpec.pg3.progs.marvel')}
              </button>
              <button
                name="other"
                onClick={skillHandler}
                className={`${skill.includes('other') ? classes.checkboxStep : classes.checkboxStepFade}`}
              >{t('formSpec.pg3.progs.other')}
              </button>
            </div>
          </motion.div>
          <motion.div
          ...
          >
            ...
            <div className={classes.checkboxWrapper}>
              <button
                name="template"
                onClick={skillHandler}
                className={`${skill.includes('template') ? classes.checkboxStep : classes.checkboxStepFade}`}
              >{t('formSpec.pg3.skills.template')}
              </button>
              <button
                name="animation"
                onClick={skillHandler}
                className={`${skill.includes('animation') ? classes.checkboxStep : classes.checkboxStepFade}`}
              >{t('formSpec.pg3.skills.animation')}
              </button>
              <button
                name="research"
                onClick={skillHandler}
                className={`${skill.includes('research') ? classes.checkboxStep : classes.checkboxStepFade}`}
              >{t('formSpec.pg3.skills.research')}
              </button>
              <button
                name="code"
                onClick={skillHandler}
                className={`${skill.includes('code') ? classes.checkboxStep : classes.checkboxStepFade}`}
              >{t('formSpec.pg3.skills.code')}
              </button>
              <button
                name="test"
                onClick={skillHandler}
                className={`${skill.includes('test') ? classes.checkboxStep : classes.checkboxStepFade}`}
              >{t('formSpec.pg3.skills.test')}
              </button>
              <button
                name="othe"
                onClick={skillHandler}
                className={`${skill.includes('othe') ? classes.checkboxStep : classes.checkboxStepFade}`}
              >{t('formSpec.pg3.skills.other')}
              </button>
            </div>
          </motion.div>
