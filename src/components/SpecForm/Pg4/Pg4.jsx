...
export default function Pg4() {
  ...
  const [skill, setSkill] = useState(['illustrator', 'animation']);
  ...
  function saveComapny() {
    const imgarr = []
    for (let i = 0; i < newimg4.length; i++) {
      imgarr.push(newimg4[i].db)
    }
    if ((imgarr.length > 0) || (compm[0]?.portfolio.length > 0)) {
      ...
      const task = {
        cost: value, price_policy: policy, checkboxes, work_type_id: 7,
      }
  ...
  useEffect(() => {
    localStorage.setItem('spec', JSON.stringify('pg4')) // i18n.changeLanguage('ru-RU');
  }, [])
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      setValue(result.data.filter(el => el.work_type_id === 7).slice(-1)[0].cost)
      setPolicy(result.data.filter(el => el.work_type_id === 7).slice(-1)[0].price_policy)
      setTerms(getObjectKeys(result.data.filter(el => el.work_type_id === 7).slice(-1)[0].checkboxes).filter(el => el === '1-3' || el === '3-6' || el === '6-12' || el === 'more')[0])
      setProg(getObjectKeys(result.data.filter(el => el.work_type_id === 7).slice(-1)[0].checkboxes))
      setSkill(getObjectKeys(result.data.filter(el => el.work_type_id === 7).slice(-1)[0].checkboxes))
      setPics(result.data.filter(el => el.work_type_id === 7).slice(-1)[0]?.portfolio)
      setPicload(false)
      setNewimg4([])
      return result
    }
        ...
        <>
          ...
          >
            <div ref={myRef} className={classes.optHeading}>
              ...
              <h3>Портфолио в иллюстрации</h3>
            </div>
            {(newimg4.length === 0 && !compm[0]) || (newimg4.length === 0 && pics?.length === 0) ? (
              <label
            ...
            {newimg4.length > 0 && <SwiperLocal img={newimg4} newimg={newimg4} setNewimg={setNewimg4} />}
            ...
            {(!newimg4 && pics?.length > 0) || pics?.length > 0 ? (
              <div className={classes.btnWrapper}>
          ...
          >
            ...
            <div className={classes.sliderDiv}>
              ...
              <input className={`${classes.inputSlider} ${classes.slider_progress}`} onChange={(e) => setValue(e.target.valueAsNumber)} value={value} type="range" max='5000' min='50' step='50' />
            </div>
            {/* {compm[0] && <SwiperTest id={compm[0].id} img={compm[0].portfolio} />} */}
          ...
          >
            ...
            <div className={classes.checkboxWrapper}>
              ...
              >{t('formSpec.pg4.progs.illustrator')}
              </button>
              <button
                name="procreate"
                onClick={skillHandler}
                className={`${skill.includes('procreate') ? classes.checkboxStep : classes.checkboxStepFade}`}
              >{t('formSpec.pg4.progs.procreate')}
              </button>
              <button
                name="photoshop"
                onClick={skillHandler}
                className={`${skill.includes('photoshop') ? classes.checkboxStep : classes.checkboxStepFade}`}
              >{t('formSpec.pg4.progs.photoshop')}
              </button>
              <button
                name="blender"
                onClick={skillHandler}
                className={`${skill.includes('blender') ? classes.checkboxStep : classes.checkboxStepFade}`}
              >{t('formSpec.pg4.progs.blender')}
              </button>
              <button
                name="cinema"
                onClick={skillHandler}
                className={`${skill.includes('cinema') ? classes.checkboxStep : classes.checkboxStepFade}`}
              >{t('formSpec.pg4.progs.cinema')}
              </button>
              <button
                name="othe"
                onClick={skillHandler}
                className={`${skill.includes('othe') ? classes.checkboxStep : classes.checkboxStepFade}`}
              >{t('formSpec.pg4.progs.othe')}
              </button>
            </div>
          ...
          >
            ...
            <div className={classes.checkboxWrapper}>
              ...
              >{t('formSpec.pg4.skills.animation')}
              </button>
              <button
                name="hero"
                onClick={skillHandler}
                className={`${skill.includes('hero') ? classes.checkboxStep : classes.checkboxStepFade}`}
              >{t('formSpec.pg4.skills.hero')}
              </button>
              <button
                name="comics"
                onClick={skillHandler}
                className={`${skill.includes('comics') ? classes.checkboxStep : classes.checkboxStepFade}`}
              >{t('formSpec.pg4.skills.comics')}
              </button>
              <button
                name="other"
                onClick={skillHandler}
                className={`${skill.includes('other') ? classes.checkboxStep : classes.checkboxStepFade}`}
              >{t('formSpec.pg4.skills.other')}
              </button>
            </div>
          </motion.div>
