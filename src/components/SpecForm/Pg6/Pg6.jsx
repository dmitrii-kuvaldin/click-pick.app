...
export default function Pg6() {
  ...
  const [skill, setSkill] = useState(['figma']);
  ...
  function saveComapny() {
    const imgarr = []
    for (let i = 0; i < newimg6.length; i++) {
      imgarr.push(newimg6[i].db)
    }
    if ((imgarr.length > 0) || (compm[0]?.portfolio.length > 0)) {
      ...
      const task = {
        cost: value, price_policy: policy, checkboxes, work_type_id: 2,
      }
      console.log('сгенерили ===>', task);
      setSaver(saver + 1)
      if (imgarr.length > 0) {
        setImg(imgarr)
        console.log('картиночки =====>>>>', imgarr);
  ...
  useEffect(() => {
    localStorage.setItem('spec', JSON.stringify('pg6')) // i18n.changeLanguage('ru-RU');
  }, [])
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      setValue(result.data.filter(el => el.work_type_id === 2).slice(-1)[0].cost)
      setPolicy(result.data.filter(el => el.work_type_id === 2).slice(-1)[0].price_policy)
      setTerms(getObjectKeys(result.data.filter(el => el.work_type_id === 2).slice(-1)[0].checkboxes).filter(el => el === '1-3' || el === '3-6' || el === '6-12' || el === 'more')[0])
      setProg(getObjectKeys(result.data.filter(el => el.work_type_id === 2).slice(-1)[0].checkboxes))
      setSkill(getObjectKeys(result.data.filter(el => el.work_type_id === 2).slice(-1)[0].checkboxes))
      setPics(result.data.filter(el => el.work_type_id === 2).slice(-1)[0]?.portfolio)
      setPicload(false)
      setNewimg6([])
      return result
    }
        ...
        <>
          ...
          >
            <div ref={myRef} className={classes.optHeading}>
              ...
              <h3>Портфолио в упаковке</h3>
            </div>
            {(newimg6.length === 0 && !compm[0]) || (newimg6.length === 0 && pics?.length === 0) ? (
              <label
            ...
            {newimg6.length > 0 && <SwiperLocal img={newimg6} newimg={newimg6} setNewimg={setNewimg6} />}
            ...
            {(!newimg6 && pics?.length > 0) || pics?.length > 0 ? (
              <div className={classes.btnWrapper}>
          ...
          >
            ...
            <div className={classes.checkboxWrapper}>
              ...
              >{t('formSpec.pg6.progs.figma')}
              </button>
              <button
                name="indesign"
                onClick={skillHandler}
                className={`${skill.includes('indesign') ? classes.checkboxStep : classes.checkboxStepFade}`}
              >{t('formSpec.pg6.progs.indesign')}
              </button>
              <button
                name="photoshop"
                onClick={skillHandler}
                className={`${skill.includes('photoshop') ? classes.checkboxStep : classes.checkboxStepFade}`}
              >{t('formSpec.pg6.progs.photoshop')}
              </button>
